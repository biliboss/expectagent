"""The eval screen. Only MonsterUI components; no custom CSS or HTML.

One name at the top — `EvalsTemplate`, the whole screen — and everything else
nests under the thing it belongs to: the two widgets under the template, their
atoms under each widget. Every class is called like a component because `__new__`
returns the element: `EvalsTemplate.RunViewWidget.Turn(t)`, never an instance to
render later.

The screen is grey on purpose. A divergence is the only coloured thing on it, so
...
class EvalsTemplate:
    ...
    class RunViewWidget:
        ...
        class Trace:
            ...
            def __new__(cls, turns: list) -> FT:
                return Steps(
                    *[EvalsTemplate.RunViewWidget.Turn(t) for t in turns],
                    cls=StepsT.vertical,
                )
            ...
            def __new__(cls, cases: list) -> FT:
                return DivVStacked(
                    *[
                        DivVStacked(
                            H4(name),
                            EvalsTemplate.RunViewWidget.Trace(turns)
        ...
        def __new__(cls, run: dict) -> FT:
            return Card(
                ...
                header=DivFullySpaced(
                    ...
                    Label(
                        run.get("verdict", "?"),
                        cls=LabelT.secondary if run.get("verdict") == "PASS" else LabelT.destructive,
                    ),
                ),
            )
        ...
        class Row:
            ...
            def __new__(cls, run: dict, index: int, selected: bool) -> FT:
                ...
                return Tr(
                    ...
                    Td(Small(EvalsTemplate.RunsWidget.Broke(run))),
                    cls="cursor-pointer",
                    hx_get=f"/?run={index}",
                    hx_target="#eval-body",
                    hx_swap="outerHTML",
    ...
    def __new__(cls, file_source: str, selected: int = 0) -> FT:
        """With no run yet it shows the spec's own turns, so the screen still says
        what is being asked of the agent instead of going blank.
        ...
        return Container(
            ...
            if runs
            else Card(cls.RunViewWidget.Cases(spec), header=H3("spec")),
            cls.RunsWidget(runs, runs[selected]) if runs else "",
            ...
            Details(Summary(Small(EVAL_FILE.name)), CodeBlock(file_source, code_cls="language-yaml")),
            id="eval-body",
        )
